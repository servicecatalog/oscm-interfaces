 ...
 * application.
 */
public class ProvisioningSettings extends ControllerSettings implements Serializable {
	 ...
	 *            the configuration settings, consisting of a key and a value each
	 * @param locale
	 *            the language. Specify a language code as returned by
	 *            <code>getLanguage()</code> of <code>java.util.Locale</code>.
	 */
	public ProvisioningSettings(HashMap<String, Setting> parameters, HashMap<String, Setting> configSettings,
			String locale) {

		this(parameters, new HashMap<String, Setting>(), new HashMap<String, Setting>(), configSettings, locale);
	}
	 ...
	 *            <code>getLanguage()</code> of <code>java.util.Locale</code>.
	 */
	public ProvisioningSettings(HashMap<String, Setting> parameters, HashMap<String, Setting> attributes,
			HashMap<String, Setting> customAttributes, HashMap<String, Setting> configSettings, String locale) {
	...
	/**
	 * Returns the name of the customer organization which created the subscription.
	 * 
	...
	/**
	 * Sets the name of the customer organization which created the subscription.
	 * 
	...
	/**
	 * Returns the platform's login page to which the application can redirect users
	 * who need to log in. This value is set for the <code>LOGIN</code> access
	 * types.
	 * <p>
	 * Typically, the login page is displayed when a user tries to directly access
	 * the application before logging in to the platform first, or when a user needs
	 * to log in again because his previous session has timed out.
	 * 
	...
	/**
	 * Sets the platform's login page to which the application can redirect users
	 * who need to log in. This value is set for the <code>LOGIN</code> access type.
	 * <p>
	 * Typically, the login page is displayed when a user tries to directly access
	 * the application before logging in to the platform first, or when a user needs
	 * to log in again because his previous session has timed out.
	...
	/**
	 * Replaces properties of configuration settings or parameters with values from
	 * attributes or parameters. In the case of attributes a prefix is used which is
	 * the controller id and an underscore. Properties will only be overwritten if
	 * the source value is not null or empty.
	 * 
	 * @param controllerId
	...
	public void overwriteProperties(String controllerId) {
		...
		overwriteProperties(getParameters(), Arrays.asList(getConfigSettings()), null);
		overwriteProperties(getCustomAttributes(), Arrays.asList(getParameters(), getConfigSettings()), controllerId);
		overwriteProperties(getAttributes(), Arrays.asList(getParameters(), getConfigSettings()), controllerId);
		overwriteCrossReferences(getParameters(), Arrays.asList(getParameters(), getConfigSettings()), null);
	}

	protected void overwriteCrossReferences(HashMap<String, Setting> source, List<HashMap<String, Setting>> targets,
			String controllerId) {
		for (Map<String, Setting> target : targets) {
			for (String key : source.keySet()) {
				...
				if (key != null && target.containsKey(key)) {
					...
					if (sourceSetting != null && sourceSetting.getValue() != null
							&& sourceSetting.getValue().matches(CR_PATTERN) && !sourceSetting.isEncrypted()
							&& (controllerId == null || controllerId.equals(sourceSetting.getControllerId()))) {

						String targetKey = tryExtract(sourceSetting.getValue());
						Setting toReplaceSetting = target.get(targetKey);
						if (toReplaceSetting != null && toReplaceSetting.getValue() != null) {
							Setting newSetting = new Setting(key, toReplaceSetting.getValue());
	...
	private void overwriteProperties(HashMap<String, Setting> source, List<HashMap<String, Setting>> targets,
			String controllerId) {
			...
			for (String key : source.keySet()) {
				...
				if (key != null && target.containsKey(key)) {
					...
					if (sourceSetting != null && sourceSetting.getValue() != null
						...
						Setting newSetting = new Setting(key, sourceSetting.getValue());
